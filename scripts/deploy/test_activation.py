...
class TransactionFailures(unittest.TestCase):
    def setUp(self):
        ...
        for name, body in {
            ...
            'node':'[[ $1 != --version ]] || { echo ${MOCK_NODE_VERSION:-v24.19.0}; exit 0; }; case "$*" in *dist/main.js*) [[ $API_PORT = 4001 && $SCHEDULERS_ENABLED = false ]] || exit 2; exec sleep 60;; *server.js*) exec sleep 60;; esac; exit 0',
    ...
    def fixture(self, base):
        for p in ['web/apps/web/server.js','web/apps/web/.next/BUILD_ID','api/dist/main.js','api/node_modules/prisma/build/index.js','api/prisma/schema.prisma','release.json','ecosystem.config.js']:
    ...
    def test_wrong_node_version_stops_before_activation(self):
        r=self.activate(MOCK_NODE_VERSION='v22.0.0')
        self.assertIn('Node v24.19.0 is required',r.stderr); self.unchanged()
        self.assertFalse((self.d/'pm2.log').exists())
    ...
    def test_switch_failure_restores_prior_release(self):
        ...
        self.assertIn('start '+str(self.releases/self.new/'ecosystem.config.js'),log)
        self.assertIn('start '+str(self.releases/self.old/'ecosystem.config.js'),log)
