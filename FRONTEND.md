...
`*-model.ts`(화면 상태/파생 로직)로 구성된다. 서비스 라우트는 API 데이터만 사용하며,
샘플 데이터와 데모 전용 화면은 `apps/web/tests/fixtures/`에 격리한다.
프로덕션 코드에서 이 폴더나 `*.mock.ts`를 import하지 않는 경계를 테스트로 검사한다.

장소 상세는 UUID 기반 실제 장소만 조회한다. 저장 코스 상세·수정과 새 일정 생성은
로그인이 필요하며, 저장 코스 조회 시 현재 요청의 세션 쿠키를 API에 전달한다.
404와 API 오류를 구분하고, 빈 목록을 샘플 데이터로 대체하지 않는다.
새 일정은 경유지가 없는 초기 상태로 시작한다. API가 제공하지 않는 평점·방문 시간·비용은
임의 생성하지 않으며, 저장 코스 편집 시 경유지의 원본 역할·주소·외부 링크를 유지한다.
