...
- 입력 검증과 모델 활성화 여부 확인 후, Bedrock 호출 전에 DB에서 원자적으로 1회를 차감한다. 시작된 요청은 오류·재시도·사용자 중단 여부와 무관하게 차감되며 환불하지 않는다. 입력 검증 실패·비활성 상태·한도 초과는 차감하지 않는다.
...
서버는 답변 전체에 55초 제한을 적용하고 초과 시 Bedrock 요청을 취소한다. 브라우저의 전체 요청 제한은 60초다. 사용자가 페이지를 떠나 취소한 스트림에는 시간 초과 안내를 띄우지 않는다. 응답 시간 초과 및 Bedrock 오류로 끝난 요청도 시작 시 차감된 사용량을 유지한다.
