...
- 운영 배포 전 CI에서 DB 예약 지연이 HTTP 최소 간격을 소모하는 회귀를 발견했다. 첫 배포는 실행되지 않았다. 응답 지연을 주입한 테스트로 재현하고 요청 종료 시각을 잠금 해제 전에 저장하도록 보강했다. 일일 예산 카운터와 스키마는 유지한다.
- 간격 보강 `13e5135`: 정책 단위 16개, PostgreSQL 회귀 10개, 전체 DB 통합 124개, API 빌드·변경 파일 린트 통과.
