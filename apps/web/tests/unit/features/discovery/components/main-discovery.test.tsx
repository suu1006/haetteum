...
import { FestivalSection } from "@/features/festivals/components/festival-section";
...
describe("MainDiscovery", () => {
  it("prioritizes only the first visible ranking row in the places tab", () => {
    const query = parseDiscoveryQuery({ tab: "places" });
      ...
      <MainDiscovery
        ...
        ranking={{ status: "ready", data: {
          ...
          items: rankingResponse.items.map((place, index) => ({
            ...place,
            primaryImageUrl: `https://tong.visitkorea.or.kr/popular-${index}.jpg`,
          })),
        } }}
        hotRanking={{ status: "ready", data: {
          ...hotRankingResponse,
          items: hotRankingResponse.items.map((place, index) => ({
            ...place,
            primaryImageUrl: `https://tong.visitkorea.or.kr/hot-${index}.jpg`,
          })),
        } }}
      />,
    );
    ...
    for (const photo of Array.from(photos).slice(0, 2)) {
      expect(photo).toHaveAttribute("loading", "eager");
      expect(photo).toHaveAttribute("fetchpriority", "high");
    }
    for (const photo of Array.from(photos).slice(2)) {
      expect(photo).toHaveAttribute("loading", "lazy");
      expect(photo).not.toHaveAttribute("fetchpriority", "high");
    }
  });
