  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Viewport className="items-center overflow-y-auto p-4">
          <Dialog.Popup
            ref={popupRef}
            initialFocus={popupRef}
            className="my-auto max-h-[calc(100dvh-2rem)] max-w-[28rem] overflow-y-auto rounded-[1.75rem] p-4 data-ending-style:scale-[0.98] data-starting-style:scale-[0.98]"
          >
            ...
            <section
              className="pt-4"
              aria-labelledby={`place-modal-${place.id}`}
            >
            ...
            <section
              className="mt-5"
              aria-labelledby={`place-reasons-${place.id}`}
            >
              ...
              <ul className="type-body-md mt-2 space-y-1.5 text-muted-foreground">
                {detail.recommendationReasons.map((reason) => (
                  <li
                    key={reason}
                    className="grid grid-cols-[0.75rem_minmax(0,1fr)] gap-1.5"
                  >
            ...
            <section
              className="mt-5"
              aria-labelledby={`place-reviews-${place.id}`}
            >
            ...
            <div className="mt-5 grid grid-cols-[0.8fr_1.2fr] gap-3">
              <Dialog.Close className="type-label min-h-12 rounded-xl border border-border bg-card px-4 text-muted-foreground transition-colors hover:bg-secondary">
                닫기
              </Dialog.Close>
              <Dialog.Close
                className="type-label min-h-12 rounded-xl bg-primary px-4 text-primary-foreground transition-[background-color,transform] hover:bg-primary-pressed active:translate-y-px"
