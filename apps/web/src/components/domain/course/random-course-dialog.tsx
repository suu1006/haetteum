...
import { LiveGeneratedCourseMap } from "@/features/courses/components/live-generated-course-map";

export type RandomCoursePhase = "loading" | "ready" | "empty";
export type SaveCourseState =
  "idle" | "saving" | "saved" | "canceling" | "error";
      ...
      <Dialog.Portal>
        ...
        <Dialog.Viewport className="items-center overflow-y-auto p-4">
          <Dialog.Popup
            ...
            className="my-auto max-w-[28rem] rounded-[1.75rem] p-4 data-ending-style:scale-[0.98] data-starting-style:scale-[0.98]"
          >
            <Dialog.Close
              aria-label="닫기"
              className="absolute top-3 right-3 inline-flex size-9 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-secondary"
            >
            ...
            {phase === "loading" ? (
              <LoadingState className="type-body-md mt-8 mb-4 text-center text-muted-foreground">
                코스를 찾는 중이에요…
              </LoadingState>
            ) : null}
